...
function sql(db: Database.Database) {
  return { kind: "sqlite" as const, db }
}

test("cursor: getCursor returns zero state for unknown streamId", () => {
  const db = freshDb()
  const state = getCursor(sql(db), "stream:1")
...
test("cursor: advanceCursor is upsert + atomic", () => {
  const db = freshDb()
  const a = advanceCursor(sql(db), "stream:1", 5, { drawerId: "abc" })
  assert.equal(a.position, 5)
  assert.equal(a.meta.drawerId, "abc")

  const b = advanceCursor(sql(db), "stream:1", 10)
  assert.equal(b.position, 10)
  // meta persists when not explicitly replaced
  assert.equal(b.meta.drawerId, "abc")

  const c = advanceCursor(sql(db), "stream:1", 15, { drawerId: "xyz", drawerCount: 42 })
...
test("cursor: advanceCursor is monotonic (rejects backward movement)", () => {
  const db = freshDb()
  advanceCursor(sql(db), "stream:1", 100)
  // Try to move backward — should be a no-op.
  const after = advanceCursor(sql(db), "stream:1", 50)
  assert.equal(after.position, 100, "position must not move backward")
})

test("cursor: resetCursor clears state but keeps the row", () => {
  const db = freshDb()
  advanceCursor(sql(db), "stream:1", 100, { x: 1 })
  incrementCounters(sql(db), "stream:1", "processed")
  incrementCounters(sql(db), "stream:1", "errors")

  const after = resetCursor(sql(db), "stream:1")
...
test("cursor: incrementCounters updates the right counter", () => {
  const db = freshDb()
  advanceCursor(sql(db), "stream:1", 1)
  incrementCounters(sql(db), "stream:1", "processed")
  incrementCounters(sql(db), "stream:1", "processed")
  incrementCounters(sql(db), "stream:1", "errors")
...
test("cursor: listCursors returns all streams sorted by recency", () => {
  const db = freshDb()
  advanceCursor(sql(db), "stream:a", 1, undefined, "2026-06-08T10:00:00Z")
  advanceCursor(sql(db), "stream:b", 1, undefined, "2026-06-08T11:00:00Z")
  advanceCursor(sql(db), "stream:c", 1, undefined, "2026-06-08T09:00:00Z")

  const all = listCursors(sql(db))
...
test("cursor: meta persists JSON-safe values across reads", () => {
  const db = freshDb()
  advanceCursor(sql(db), "stream:1", 1, { items: [1, 2, 3], nested: { ok: true }, str: "hello" })
  const state = getCursor(sql(db), "stream:1")
...
test("cursor: corrupt meta is tolerated (logged warn, returned empty)", () => {
  ...
  const state = getCursor(sql(db), "stream:bad")
  assert.deepEqual(state.meta, {})
})
