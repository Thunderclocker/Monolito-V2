...
function buildSystemPrompt(args: {
  ...
  const staticSystem = [
    ...
    "- SessionForensics returns toolInvocationCount when the question names a tool; if counts/evidence are incomplete, say forensics was partial and widen worklogLimit — never deny a tool call from a truncated sample.",
    "- tool_manage_config get/read never changes configuration. Do not re-set CONF_CHANNELS.telegram when token, enabled, and allowedChats are already correct unless the user explicitly asks to reconfigure.",
