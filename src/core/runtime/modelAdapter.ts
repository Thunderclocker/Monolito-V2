  ...
  const staticSystem = [
    ...
    isSubAgent
      ...
      : [
          ...
          "",
          "USER-PROFILE-OVERRIDES-CONTEXT RULE (CRITICAL):",
          "- Persistent user facts registered in BOOT_USER (location, timezone, preferred language, name, occupation, family, recurring preferences, etc.) take ABSOLUTE PRECEDENCE over any data that appears in conversation context, recent search results, recent tool outputs, or your own memory of past turns.",
          "- When the user asks a question that depends on a user-specific fact (weather, news, local events, business hours, traffic, public transport, store availability, local regulations, language for a response, etc.) WITHOUT explicitly specifying the fact, you MUST use the value registered in BOOT_USER. You are FORBIDDEN from defaulting to:",
          "  * Cities, countries, or regions mentioned in recent searches",
          "  * Cities, countries, or regions mentioned in recent tool outputs",
          "  * The capital of a country when the user lives in a different city",
          "  * Languages from past conversation turns if the user has a different preferred language in BOOT_USER",
          "  * Any data point that is 'more recent' than the registered user fact",
          "- The right pattern is: read BOOT_USER → if the fact is there, use it; if the user wants a different one, they will say so explicitly.",
          "- If a fact is NOT in BOOT_USER and the user does not specify it, you may either ask the user OR make a reasonable default — but never silently substitute a different value from conversation context.",
          "- Wrong example: 'está pronosticado lluvia para hoy?' → assistant answers with Buenos Aires because the most recent web search was about BA. Correct: assistant uses BOOT_USER.location (e.g. 'Santo Tomé, Santa Fe') to scope the search.",
          "- Wrong example: user with preferred_language='en' in BOOT_USER asks something general → assistant answers in Spanish because the last 5 turns were in Spanish. Correct: assistant uses BOOT_USER.preferred_language='en' for the response.",
        ].join("\n"),
