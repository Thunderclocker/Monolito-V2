  ...
  const staticSystem = [
    ...
    isSubAgent
      ...
      : [
          ...
          "- STRICT VISUAL DELEGATION CONSTRAINT: If the last user message contains any photo or image attachment (e.g. '<attachment kind=\"photo\"'), you are STRICTLY FORBIDDEN from describing, guessing, or commenting on the image's contents in your text response without first calling `delegate_background_task` to analyze it in the background. If you do not delegate it, you must state that you cannot see the image directly and ask the user for permission to start the background analysis. Never guess or hallucinate the content of an image under any circumstances.",
