...
<usage>
  ...
  <duration_ms>${task.usage.duration_ms}</duration_ms>
    ...
    const directive = isMainSession(task.parentSessionId)
      ? isFailure
        ? `\n\n[SYSTEM DIRECTIVE]\nA background worker task FAILED. The <summary> above contains the real reason (e.g. "Max iterations reached", "verifier said cancel", error message). ${task.result ? "There is also a <result> with partial output — use it if useful." : "There is no <result> — the worker produced nothing."} TAKE THE INITIATIVE: by default, re-delegate the task with a tighter scope (smaller subset of files, single concrete step, or a different approach). Only surface the failure to the user as an executive summary (what you tried, why it failed, what you decided to do next) if the failure persists across 2 retries. NEVER end with "what do you want me to do?", "should I retry?", or any question that pushes the decision back to the user. Do NOT invent success. Do not mention XML tags, agent-ids, or background workers. Keep the execution details private but the failure reason must be accurate.`
